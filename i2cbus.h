...
public:
	static ErrorType initializeBus(const i2c_master_bus_config_t &buscfg);
	static I2CBus *get(const i2c_port_t &port);
	static ErrorType shutdown(const i2c_port_num_t &port);
public:
	~I2CBus();
	const i2c_port_num_t &getPortID() const {return BusConfig.i2c_port;}
	...
	ErrorType shutdown();
protected:
	I2CBus(const i2c_master_bus_config_t &buscfg, i2c_master_bus_handle_t handle);
private:
