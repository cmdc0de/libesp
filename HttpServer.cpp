 ...
 */
class libesp::HttpServerTask: public Task {
public:
...
private:
	libesp::HttpServer* m_pHttpServer; // Reference to the HTTP Server
	 ...
	 */
	void processRequest(libesp::HttpRequest &request) {
		...
		for (auto pathHandlerIterartor = m_pHttpServer->m_pathHandlers.begin();
			...
			if (pathHandlerIterartor->match(request.getMethod(), request.getPath())) { // Did we match the handler?
				...
				if (request.isWebsocket()) {                                     // Is this handler to be invoked for a web socket?
					...
					libesp::HttpResponse response(&request);
	 ...
	 */
	void run(void* data) {
		m_pHttpServer = (libesp::HttpServer*)data;             // The passed in data is an instance of an HttpServer.
		...
		while(1) {   // Loop forever.
			...
			libesp::HttpRequest request(clientSocket);   // Build the HTTP Request from the socket.
 ...
 */
void libesp::HttpServer::addPathHandler(
		std::string method,
		std::regex* pathExpr,
		void (*handler)(libesp::HttpRequest *pHttpRequest, HttpResponse *pHttpResponse)) {
 ...
 */
size_t libesp::HttpServer::getFileBufferSize() {
 ...
 */
uint16_t libesp::HttpServer::getPort() {
	return m_portNumber;
} // getPort
 ...
 */
std::string libesp::HttpServer::getRootPath() {
	return m_rootPath;
} // getRootPath
 ...
 */
void libesp::HttpServer::listDirectory(std::string path, HttpResponse& response) {
 ...
 */
void libesp::HttpServer::setClientTimeout(uint32_t timeout) {
 ...
 * @param [in] use Set to true to enable directory listing.
 */
uint32_t libesp::HttpServer::getClientTimeout() {
 ...
 * @param [in] use Set to true to enable directory listing.
 */
void libesp::HttpServer::setDirectoryListing(bool use) {
 ...
 */
void libesp::HttpServer::setFileBufferSize(size_t fileBufferSize) {
 ...
 * @return N/A.
 */
void libesp::HttpServer::setRootPath(std::string path) {
 ...
 */
void libesp::HttpServer::start(uint16_t portNumber, bool useSSL) {
	// Design:
	...
	libesp::HttpServerTask* pHttpServerTask = new libesp::HttpServerTask("HttpServerTask");
 ...
 */
libesp::PathHandler::PathHandler(std::string method, std::regex *pRegex,
		void (*pWebServerRequestHandler)
		(
			libesp::HttpRequest*  pHttpRequest,
 ...
 * @param [in] webServerRequestHandler The request handler to be called.
 */
libesp::PathHandler::PathHandler(std::string method, std::string matchPath,
		void (*pWebServerRequestHandler)
		(
			libesp::HttpRequest*  pHttpRequest,
			HttpResponse* pHttpResponse)
 ...
 */
bool libesp::PathHandler::match(std::string method, std::string path) {
	if (method != m_method) {
		return false;
	}
	if (m_isRegex) {
 ...
 * @return N/A.
 */
void libesp::PathHandler::invokePathHandler(libesp::HttpRequest* request, HttpResponse *response) {
