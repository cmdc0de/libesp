...
const int libesp::HttpResponse::HTTP_STATUS_CONTINUE              = 100;
const int libesp::HttpResponse::HTTP_STATUS_SWITCHING_PROTOCOL    = 101;
const int libesp::HttpResponse::HTTP_STATUS_OK                    = 200;
const int libesp::HttpResponse::HTTP_STATUS_MOVED_PERMANENTLY     = 301;
const int libesp::HttpResponse::HTTP_STATUS_BAD_REQUEST           = 400;
const int libesp::HttpResponse::HTTP_STATUS_UNAUTHORIZED          = 401;
const int libesp::HttpResponse::HTTP_STATUS_FORBIDDEN             = 403;
const int libesp::HttpResponse::HTTP_STATUS_NOT_FOUND             = 404;
const int libesp::HttpResponse::HTTP_STATUS_METHOD_NOT_ALLOWED    = 405;
const int libesp::HttpResponse::HTTP_STATUS_INTERNAL_SERVER_ERROR = 500;
const int libesp::HttpResponse::HTTP_STATUS_NOT_IMPLEMENTED       = 501;
const int libesp::HttpResponse::HTTP_STATUS_SERVICE_UNAVAILABLE   = 503;

static std::string lineTerminator = "\r\n";
libesp::HttpResponse::HttpResponse(libesp::HttpRequest *request) {
	m_request = request;
	m_status  = 200;
	m_headerCommitted = false; // We have not yet sent a header.
}

libesp::HttpResponse::~HttpResponse() {
}
 ...
 */
void libesp::HttpResponse::addHeader(const std::string name, const std::string value) {
 ...
 * the socket.
 */
void libesp::HttpResponse::close() {
 ...
 */
std::string libesp::HttpResponse::getHeader(std::string name) {
...
std::map<std::string, std::string> libesp::HttpResponse::getHeaders() {
 ...
 */
void libesp::HttpResponse::sendData(std::string data) {
...
void libesp::HttpResponse::sendData(uint8_t* pData, size_t size) {
...
void libesp::HttpResponse::sendFile(std::string fileName, size_t bufSize)
{
	...
	if (!ifStream.is_open()) {
		...
		setStatus(libesp::HttpResponse::HTTP_STATUS_NOT_FOUND, "Not Found");
		addHeader(libesp::HttpRequest::HTTP_HEADER_CONTENT_TYPE, "text/plain");
		sendData("Not Found");
		close();
	...
	setStatus(libesp::HttpResponse::HTTP_STATUS_OK, "OK");
 ...
 */
void libesp::HttpResponse::sendHeader() {
	// If we haven't yet sent the header of the data, send that now.
	if (m_headerCommitted == false) {
 ...
 */
void libesp::HttpResponse::setStatus(const int status, const std::string message) {
