 ...
 */
libesp::Socket libesp::Socket::accept() {
 ...
 */
std::string libesp::Socket::addressToString(struct sockaddr* addr) {
 ...
 */
int libesp::Socket::bind(uint16_t port, uint32_t address) {
 ...
 */
int libesp::Socket::connect(struct in_addr address, uint16_t port) {
	struct sockaddr_in serverAddress;
	serverAddress.sin_family = AF_INET;
 ...
 * @return Success or failure of the connection.
 */
int libesp::Socket::connect(char* strAddress, uint16_t port) {
 ...
 */
int libesp::Socket::createSocket(bool isDatagram) {
 ...
 * @return N/A.
 */
void libesp::Socket::getBind(struct sockaddr* pAddr) {
	if (m_sock == -1) {
 ...
 */
int libesp::Socket::getFD() const {
	return m_sock;
} // getFD

bool libesp::Socket::getSSL() const {
	return m_useSSL;
}

bool libesp::Socket::isValid() {
	return m_sock != -1;
} // isValid
 ...
 * @return Returns 0 on success.
 */
int libesp::Socket::listen(uint16_t port, bool isDatagram, bool reuseAddress) {
...
bool libesp::Socket::operator <(const Socket& other) const {
 ...
 */
int libesp::Socket::setSocketOption(int option, void* value, size_t len)
{
 ...
 */
int libesp::Socket::setTimeout(uint32_t seconds)
{
    struct timeval tv;
    tv.tv_sec = seconds;
    tv.tv_usec = 0;
...
std::string libesp::Socket::readToDelim(std::string delim) {
	std::string ret;
	std::string part;
	auto it = delim.begin();
	while(1) {
 ...
 */
size_t libesp::Socket::receive(uint8_t* data, size_t length, bool exact) {
 ...
 */
int libesp::Socket::receiveFrom(uint8_t* data, size_t length,	struct sockaddr *pAddr) {
 ...
 *
 */
int libesp::Socket::send(const uint8_t* data, size_t length) const {
 ...
 */
void libesp::Socket::sendTo(const uint8_t* data, size_t length, struct sockaddr* pAddr) {
	int rc;
	if (getSSL()) {
 ...
 */
void libesp::Socket::setReuseAddress(bool value) {
 ...
 */
void libesp::Socket::setSSL(bool sslValue) {
 ...
 */
void libesp::Socket::sslHandshake() {
 ...
 */
std::string libesp::Socket::toString() {
 ...
 */
libesp::SocketInputRecordStreambuf::SocketInputRecordStreambuf(
	Socket  socket,
	size_t  dataLength,
	size_t  bufferSize) {
 ...
 *
 */
libesp::SocketInputRecordStreambuf::int_type libesp::SocketInputRecordStreambuf::underflow() {
...
libesp::SocketException::SocketException(int myErrno) {
	m_errno = myErrno;
}
